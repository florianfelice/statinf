...
sys.path.append(f"/Users/{getpass.getuser()}/Documents/Projects/statinf/")

try : 
    from statinf.stats.supervised import GGM
except:
    from statinf.stats.supervised import GGM

from statinf.stats.unsupervised import GaussianMixture
from statinf.regressions.LinearModels import Linear_bayesian
...
def test_ggm(norm = "mahalanobis",isotropic = True):
    if isotropic :  
        if norm == "euclidian":
            ...
            # X_class_1 = np.random.rand(nb_samples,2)@covariance_matrix + [5,5]
            # X_class_2 = np.random.rand(nb_samples,2)@covariance_matrix + [-1,11]
            # X_class_3 = np.random.rand(nb_samples,2)@covariance_matrix + [0,0]
        ...
        nb_samples = 500
        X, labels = make_blobs(n_samples=[nb_samples, nb_samples,nb_samples], cluster_std= [0.5,1,2], centers=[[2,2],[-5,-5],[2,2]], n_features=3, \
                        random_state = 0)
...
def test_linear_bayesian(true_sigma = 10):
    ...
    def generate_linear_data(n_samples  = 50,
                            bias       = 0,
                            weights    = 5,
                            true_sigma = 10
                            ):
